...
@section('content')
    <div class="container-fluid">
                            ...
                            <div class="form-group col-6">
                                ...
                                <select class="custom-select" id="product-selector">
                                    ...
                                    @foreach($products as $product)
                                        <option data-unit="{{ $product->unit_id }}" value="{{ $product->id }}">{{ $product->title }}</optiondata-unit>
                                    @endforeach
                                </select>
                            </div>
                        </div>
        ...
        <div class="row justify-content-center">
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header bg-light">{{ __('ویرایش خروج از انبار') }}</div>

                    <div class="card-body">
                        <form id="destroyForm" action="{{ route('outputs.destroy', ['output' => $receipt->id]) }}" method="post"></form>

                        <form id="new-entry" method="post" action="{{ route('outputs.update', ['output' => $receipt->id]) }}">
                            <div class="form-group row">
                                ...
                                <div class="form-group col-2">
                                    ...
                                    <input type="text" id="worker" name="worker"
                                           ...
                                           value="{{ $receipt->worker->title }}" required>
                                    <input type="hidden" id="worker-id" name="worker_id"
                                           value="{{ $receipt->worker_id }}">
                                </div>
                                <div class="form-group col-2">
                                        <label for="part">محل مصرف</label>
                                        <input type="text" id="part" name="part"
                                               class="form-control text-dark bg-white form-control-sm" value="{{ $receipt->part->title ?? '' }}" required>
                                        <input type="hidden" id="part-id" name="part_id" value="{{ $receipt->part_id }}">
                                    </div>
                                <div class="form-group col-4">
                                    ...
                                    <a target="_blank" href="{{ url('/') }}/outputs/print/{{ $receipt->id }}" class="btn btn-primary float-left w-25 ml-3 mt-3"><i class="fa fa-print"></i> چاپ</a>
                                </div>
                            </div>

                            @foreach($receipt->outputs as $output)
                                ...
                                <div class="form-group row custom-single-product">
                                    ...
                                    <div class="form-group col-3">
                                        ...
                                        <input type="text" id="product" name="product"
                                               class="form-control text-dark bg-white form-control-sm" value="{{ $output->product->title }}" required>
                                        <input type="hidden" id="product-id" name="product_id[]" value="{{ $output->product_id }}">
                                    </div>
                                    ...
                                    <div class="form-group col-2">
                                        ...
                                        <select class="custom-select custom-select-sm" id="unit" name="unit[]" required>
                                            ...
                                            @foreach($units as $unit)
                                                <option class="category-level-one"
                                                        value="{{ $unit->id }}" @if($unit->id == $output->unit_id) selected @endif>
                                                    {{ $unit->title }}</option>
                                            @endforeach
                                        </select>
                                    </div>
